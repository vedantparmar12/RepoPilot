...
const logger = (0, logger_1.createLogger)('GitHubClient');
class GitHubClient {
    octokit;
    rateLimiter;
    constructor(token) {
        ...
        if (!authToken) {
            throw new error_handler_1.AuthenticationError('GitHub token is required. Set GITHUB_TOKEN environment variable or pass token to constructor.');
        }
    ...
    async listPullRequestFiles(owner, repo, pull_number, page = 1, per_page = 30) {
        return this.rateLimiter.withBackoff(async () => {
    ...
    async updateIssue(params) {
        return this.rateLimiter.withBackoff(async () => {
            ...
            if (params.milestone !== undefined)
                updateData.milestone = params.milestone;
